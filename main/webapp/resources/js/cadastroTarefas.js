...
function editaRegistro(html) {
    ...
    if (taskInEdit) {
        return FLUIGC.toast({
            ...
            message: 'Somente é permitido a edição de um registro por vez.',
            type: 'danger'
        });
    }
    ...
    var element = document.getElementById('rw_formCadTask');

    while (element.firstChild) {
        element.removeChild(element.firstChild);
    }

    if ($('#body_categorias').children().length > 0) {
        ...
        let slaUserValue = paramsEdit.slaUser;
        let numberMatch = slaUserValue.match(/\d+/);

        if (numberMatch) {
            paramsEdit.slaUser = parseInt(numberMatch[0], 10);
        }

        addTask('Editar', paramsEdit)
    }
}

var docInEdit

function templateTask(type, numTask, paramsEdit) {

    console.log(paramsEdit)
    var titulo = 'Tarefa'
    if (type == 'Editar') {
        titulo = 'Editar'
        docInEdit = paramsEdit.DOCUMENTID
    }

    var template = `
    <div class="row" id="rw_rowFormCad___${numTask}" class="taskGrid">
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4 class="panel-title"">
                                <i class="flaticon flaticon-trash icon-md" aria-hidden="true" onclick="removeTask(${numTask})" style="display: inline;"></i>
                    ...
                    <div class="panel-body">
                        <div class="row" id="rw_taskConfig___${numTask}">
                            <div class="col-md-6 form-group">
                                ...
                                <input type="hidden" id="hd_form_formCadTask_category___${numTask}" name="hd_formCadTask_category___${numTask}" value="${paramsEdit ? paramsEdit.filter_categoria.TXT_ID_CATEGORIA : ''}"/>
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="hidden" id="hd_formCadTask_typeTask___${numTask}" name="hd_formCadTask_typeTask___${numTask}" value="${paramsEdit ? paramsEdit.filter_tpAtendimento.TXT_ID_TIPOATEND : ''} "/>
                            </div>
                        </div>
                        <div class="row" id="rw_taskForm___${numTask}">
                            <div class="col-md-6 form-group">
                                <label for="txt_formCadTask_taskName___${numTask}">Tarefa</label>
                                <input name="txt_formCadTask_taskName___${numTask}" id="txt_formCadTask_taskName___${numTask}" class="form-control" value="${paramsEdit ? paramsEdit.descricao_tarefa : ''}"/>
                            </div>
                            <div class="col-md-3 form-group">
                                <label for="txt_formCadTask_type___${numTask}">Tipo</label>
                                <select id="txt_formCadTask_type___${numTask}" class="form-control" value="${paramsEdit ? paramsEdit.tipo_solic : 'Incidente'} ">
                            ...
                            <div class="col-md-3 form-group">
                                ...
                                <input name="txt_formCadTask_sla___${numTask}" id="txt_formCadTask_sla___${numTask}" type="number" class="form-control" value="${paramsEdit ? paramsEdit.slaUser : ''}"/>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-4">
    ...
    return { template, numTask }
}

function createFilterTask(type, numTask) {
    if (type == 'category') {
        ...
        var findFilterCat = filterCat.findIndex(item => item.id == `#ztxt_formCadTask_category___${numTask}`)
        if (findFilterCat == -1) {
            filterCat.push({
...
var filterCat = [], filterAtend = []
var tasksQtd = 0

function addTask(type, paramsEdit) {

    tasksQtd += 1
    var numTask = tasksQtd
    var task = templateTask(type, numTask, paramsEdit)

    $("#rw_formCadTask").append(task.template)

    createFilterTask('category', task.numTask)

    if (type == 'Editar') {
        createFilterTask('atend', task.numTask)

        var index = filterCat.findIndex(item => item.id == `#ztxt_formCadTask_category___${task.numTask}`)

        filterCat[index].filter.add({
            TXT_CATEGORIA: paramsEdit.filter_categoria.TXT_CATEGORIA,
            TXT_ID_CATEGORIA: paramsEdit.filter_categoria.TXT_ID_CATEGORIA
        })

        filterAtend[index].filter.add({
            TXT_TIPO_ATENDIMENTO: paramsEdit.filter_tpAtendimento.TXT_TIPO_ATENDIMENTO,
            TXT_ID_TIPOATEND: paramsEdit.filter_tpAtendimento.TXT_ID_TIPOATEND
        })

        var c1 = DatasetFactory.createConstraint('DOCUMENTID', paramsEdit.DOCUMENTID, paramsEdit.DOCUMENTID, ConstraintType.MUST)

        var descriptions = DatasetFactory.getDataset('dts_consultaPaiXFilhosTarefasMFX', null, [c1], null)

        for (var i = 0; i < descriptions.values.length; i++) {

            var rowDesc = `<div id="rw_tabCadDesc___${task.numTask}___${i}" name="rw_tabCadDesc___${task.numTask}___${i}" class="row">
                                <div class="col-md-5">
                                    <label for="">Descrição</label>
                                    <input type="text" id="txt_descName___${task.numTask}___${i}" name="txt_descName___${task.numTask}___${i}" value="${descriptions.values[i].TXT_DESCRICAO}" class="form-control question" />
                                </div>
                        </div>
                        `
            $(`#rw_rowFormCadTask___${task.numTask}`).append(rowDesc)
        }
    }
}
...
function addDesc(value) {
    ...
    var row = `
        <div id="rw_tabCadDesc___${value}___${newRow}" name="rw_tabCadDesc___${value}___${newRow}" class="row">
            <div class="col-md-5">
                <label for="">Descrição</label>
                <input type="text" id="txt_descName___${value}___${newRow}" name="txt_descName___${value}___${newRow}" class="question" />
            </div>
        </div>
    `
...
function taskData(numTask) {
    ...
    var position = 5
    taskInEdit ? position = 4 : position = 5
    ...
    dataForm =
        [ // lista dos campos 
            ...
            {
                "name": "hd_nome_tpAtend",
                "value": $(`#rw_taskConfig___1`)[0].querySelectorAll('input')[position].value // Quando é edit, a posição é 4, qnd é salvar, a posição é 5
            },
            {
                "name": "txt_tarefa",
...
//Função responsável para pegar o ID das Atividades.
function getIds() {
    var tasks = $("#rw_formCadTask").children();
    var tasksIds = []
    tasks.each(function (index, task) {
        var taskId = $(task).attr('id');
        tasksIds.push(taskId.split("___")[1])
    });
...
async function saveTasks() {
    ...
    for (let i = 0; i < taskIds.length; i++) {
        ...
        await saveTask(taskIds[i]);
    }

    const instance = new Ged()
    docInEdit != '' ? instance.delete(docInEdit) : ''
...
function removeTask(numTask) {
    if (taskInEdit) {
        taskInEdit = false
    }
    $(`#rw_rowFormCad___${numTask}`).remove();
}